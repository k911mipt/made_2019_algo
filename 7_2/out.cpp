...
int main() {
    ...
    for (size_t i = 0; i < n; ++i) {
        std::cin >> command >> val;
        if (command == 1) {
            //std::cout << tree.InsertSplay(val) << std::endl; // Вставка с балансировкой при поиске места вставки
            std::cout << tree.Insert(val) << std::endl; // Вставка с балансировкой после добавления элемента
        }
        else {
            //tree.DeletePos(val);
            tree.Delete(tree.FindKOrder(val));
        }
    }
    return EXIT_SUCCESS;
}
